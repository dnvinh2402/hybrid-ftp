...
struct RDTHeader
{
    ...
    uint8_t reserved;     // compression, encryption, priority
...
inline uint16_t computeChecksum(const uint8_t *data, size_t len)
{
    ...
    if (i < len)
        sum += (data[i] << 8); // byte le cuoi cung
    while (sum >> 16)
...
inline size_t packForSend(const RDTPacket &pkt, uint8_t *outBuf, size_t outBufCap)
{
    ...
    if (total > outBufCap)
        return 0; // buffer dua vao khong du cho
